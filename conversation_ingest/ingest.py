...
def extract_signal(text: str, conversation_id: str, timestamp: str) -> dict[str, Any]:
    ...
    return {
        ...
        "timestamp":        timestamp,
        "decisions":        [ln.strip() for ln in lines if any(m in ln.lower() for m in DECISION_MARKERS)][:20],
        "promises":         [ln.strip() for ln in lines if any(m in ln.lower() for m in PROMISE_MARKERS)][:20],
        "deadlines":        [ln.strip() for ln in lines if any(m in ln.lower() for m in DEADLINE_MARKERS)][:10],
        "projects_named":   list({p.upper() for p in KNOWN_PROJECTS if p in lower}),
        "emotional_markers": [em for em in EMOTION_MARKERS if em in lower],
        "tech_stack":       list({tk for tk in TECH_KEYWORDS if tk in lower}),
        "open_loops":       [ln.strip() for ln in lines if any(m in ln.lower() for m in LOOP_MARKERS)][:30],
